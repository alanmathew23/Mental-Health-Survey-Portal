    ...
    def login(self):
        ...
        if role == "student":
            ...
            try:
                ...
                result = DatabaseManager.fetch_data(query, (student_id,password))
            except ValueError:
                query = "SELECT * FROM Student WHERE Email = %s AND Password = %s"
                result = DatabaseManager.fetch_data(query, (email,password))
            ...
            try:
                ...
                query = "SELECT * FROM Counselor WHERE CounselorID = %s AND Password = %s"
                result = DatabaseManager.fetch_data(query, (counselor_id,password))
            except ValueError:
                query = "SELECT * FROM Counselor WHERE Email = %s AND Password = %s"
                result = DatabaseManager.fetch_data(query, (email,password))
...
class StudentApp(tk.Tk):
    ...
    def book(self):
        try:
            if not all([self.counselor_var.get()]):
                messagebox.showerror("Error", "Please select a counselor")
                return
            ...
            query = """
                INSERT INTO Session (StudentID, CounselorID, SessionDate, StartTime, EndTime, Notes)
                VALUES (%s, %s, %s, %s, %s, %s)
            """
            params = (
                self.student['StudentID'],
